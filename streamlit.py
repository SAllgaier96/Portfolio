...
st.json({top_host_1.iloc[0].host_name: top_host_1    [["name", "neighbourhood", "room_type", "minimum_nights", "price"]]        .sample(2, random_state=4).to_dict(orient="records"),
...
st.write("💡 Notice how we use a static table below instead of a data frame. Unlike a data frame, if content overflows out of the section margin, a static table does not automatically hide it inside a scrollable area. Instead, the overflowing content remains visible.")
...
st.write("At 169 days, Brooklyn has the lowest average availability. At 226, Staten Island has the highest average availability.    If we include expensive listings (price>=$200), the numbers are 171 and 230 respectively.")
st.markdown("_**Note:** There are 18431 records with `availability_365` 0 (zero), which I've ignored._")

df.query("availability_365>0").groupby("neighbourhood_group")    .availability_365.mean().plot.bar(rot=0).set(title="Average availability by neighborhood group",
...
st.write("486 is the highest number of reviews and two properties have it. Both are in the East Elmhurst     neighborhood and are private rooms with prices $65 and $45.     In general, listings with >400 reviews are priced below $100.     A few are between $100 and $200, and only one is priced above $200.")
st.header("Images")
pics = {
