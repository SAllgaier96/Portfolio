...
grid.arrange(arr_flights, arr_cancelled, arr_delay, mkval, ncol=2)



#Airlines Covid Model: 
SW_model_covid = lm(MarketCap~arr_flights_sum+arr_del15_sum+arr_cancelled_sum+covid, data= SW)
summary(SW_model_covid)
par(mfrow = c(2,2))
plot(SW_model_covid)

DL_model_covid = lm(MarketCap~arr_flights_sum+arr_del15_sum+arr_cancelled_sum+covid, data= DL)
summary(DL_model_covid)
par(mfrow = c(2,2))
plot(DL_model_covid)

UA_model_covid = lm(MarketCap~arr_flights_sum+arr_del15_sum+arr_cancelled_sum+covid, data= UA)
